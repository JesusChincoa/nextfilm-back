...
ruta.get('/getRentals', verificarTokenAdmin ,(req, res) => {
    ...
    result.then((rentals) => res.status(200).json(rentals))
...
ruta.get('/getRental/:id', verificarTokenAdmin ,(req, res) => {
...
ruta.get('/getRentalsByUser/:userId', verificarTokenAdmin ,(req, res) => {
...
ruta.post('/newRental', async (req, res) =>{
    let body = req.body
    ...
    if(!existingFilm) return res.status(400).json({
        errorNumber: 400,
        error: 'Pelicula no encontrada'
    })

    let existingRental = await rentalService.searchExistingRental(body.userId, body.filmId);
    if(existingRental) return res.status(400).json({
        errorNumber: 400,
...
ruta.put('/updateRental/:id', verificarTokenAdmin , async (req, res) => {
    ...
    try{
        if(!rental) return res.status(400).json({
        errorNumber: 400,
        error: 'Alquiler no encontrado'
        })
        const {error} = schema.validate({
